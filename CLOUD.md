...
- [x] **P1 Maxi as a workflow + per-person Cart + Ideas + packaging at checkout** — ✅ CODE COMPLETE (Aug 2026). The agent backend already existed (`POST /maxi`, 13 tools, memory, budgets) but was reachable only through the Home search bar's **mic**, its transcript died with the sheet, and the `actions` it emitted were decoded and thrown away. Five changes:
  ...
  (6) **Age-group idea packs** — KNOWLEDGE is relationship-keyed with no age partitions, so 7 bands (5–8 / 9–12 / 13–17 / 18–25 / 26–39 / 40–54 / 55+) are hand-authored in `build-shelves.mjs` and resolved through the EXISTING `resolveBundleSlot()`, with an `ADULT_ONLY_RE` guard so a kNN neighbour of "kids art supplies" can't be a wine-themed craft kit. Written as ordinary `bundles#age-*` rows ⇒ **`GET /bundles` serves them unchanged**. `buildAgePacks` pushes into the same `recipients` array before `bundles#index` is written — otherwise the next run silently drops them from the sampler.
  ...
  **Verified:** `npm --prefix infra/src test` 45/45 · iOS build + 74/74 tests · `terraform validate` clean.
  ⏳ **AWS-GATED (infra agent):** (a) `terraform apply` — Nova Canvas IAM statement, `wrap/public/*` on the media bucket policy + a CloudFront behaviour, and the four `PACKAGING_*` env vars in BOTH `lambda.tf` and `apprunner.tf`. (b) Deploy the Lambda **and** App Runner (`handler.mjs` + `packaging.mjs`). (c) `node infra/ingest/build-shelves.mjs --bundles --dry-run` then without `--dry-run` to publish the age packs. (d) **Console: request Bedrock model access for `amazon.nova-canvas-v1:0`**, then flip `packaging_images = true` — until then the route ships plans without pictures, by design. (e) Claude Haiku 4.5 access is still ungranted (`terraform.tfvars:10` pins Nova Lite); the brief tools work either way but get materially better on Haiku.
