...
# Packaging at checkout (POST /packaging). The PLAN model is multimodal and
# already granted (it serves Maxi); only the IMAGE model needs a new Bedrock
# console model-access grant — until then leave PACKAGING_IMAGES=0 and the
# route returns plans without pictures.
PACKAGING_VISION_MODEL_ID=          # empty -> reuse MAXI_BASE_MODEL_ID (Nova Lite)
PACKAGING_IMAGE_MODEL_ID=amazon.nova-canvas-v1:0
PACKAGING_IMAGES=0
PACKAGING_DAILY_LIMIT=20
...
### 12.3 Apify scheduled-ingest path

The operator path is live now: Apify Actor run → `ingest-apify-pinterest.mjs` →
precheck/editorial review → S3 + DynamoDB → CloudFront `/feed` → iOS. Harden it
without giving Apify AWS credentials:

1. Save the Pinterest Actor input as an Apify Task and schedule it in Apify.
2. Send the successful-run webhook to a signed `POST /ingest/apify` endpoint.
3. Validate the webhook secret, store `{runId,datasetId}` as the idempotency key,
   then enqueue it to SQS with a DLQ.
4. A single-purpose curator Lambda fetches the dataset, runs the same prechecks,
   writes the immutable images/report to S3, and upserts the approved posts.
5. Alarm on DLQ depth and rejected/accepted ratio drift; keep the human `picks`
   checkpoint for editorial carousels until scoring quality is stable.
