...
// Onboarding IS the Gift Concierge (mirrors web/components/app/gift-consult.tsx).
//
// New users don't page through marketing slides — they meet Maxi and run a
// real consult immediately: who are we gifting, what's their world, budget
// OPTIONAL ("just find the gift" is a first-class answer). The consult ends
// in a real pick from the catalog with two doors:
...
private final class ConsultViewModel: ObservableObject {
    enum Phase: Int {
        case intro, relation, name, occasion, budget, worlds, keeper, thinking, results
    }
    ...
    private var senderId: String { InteractionQueue.anonymousUserId }
...
struct OnboardingView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            switch vm.phase {
            case .intro: IntroStep { vm.phase = .relation }
            case .thinking: ThinkingStep(vm: vm)
            case .results: ResultsStep(vm: vm, finish: complete)
            default: questionSteps
            }
        }
        .background(Color(hex: "#FFF9F5").ignoresSafeArea())
        .interactiveDismissDisabled(vm.phase != .results)
    }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                PrimaryButton("Start exploring the app →") { finish() }
                    .padding(.top, 8)
            }
            .padding(20)
        }
